...
function Enable-PackageDownloader{
    ...
    process{
        if(!(get-module package-downloader)){            
            ...
            $modPath = (join-path $toolsDir 'package-downloader.1.0.1-beta5\tools\package-downloader.psm1')
            if(!(Test-Path $modPath)){
                $nugetArgs = @('install','package-downloader','-prerelease','-version','1.0.1-beta5','-OutputDirectory',(Resolve-Path $toolsDir).ToString())
