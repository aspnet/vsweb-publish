...
function Enable-PackageDownloader{
    ...
    process{
        if(!(get-module package-downloader)){            
            ...
            $modPath = (join-path $toolsDir 'package-downloader.1.1.1\tools\package-downloader.psm1')
            if(!(Test-Path $modPath)){
                $nugetArgs = @('install','package-downloader','-prerelease','-version','1.1.1','-OutputDirectory',(Resolve-Path $toolsDir).ToString())
