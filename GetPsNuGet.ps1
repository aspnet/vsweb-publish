...
function Enable-PsNuGet{
    ...
    process{
        if(!(get-module 'getnuget')){            
            ...
            $modPath = (join-path $toolsDir 'getnuget.0.0.11-beta\tools\getnuget.psm1')
            if(!(Test-Path $modPath)){
                $nugetArgs = @('install','getnuget','-prerelease','-version','0.0.11-beta','-OutputDirectory',(Resolve-Path $toolsDir).ToString())
