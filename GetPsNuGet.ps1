...
function Enable-PsNuGet{
    ...
    process{
        if(!(get-module 'getnuget')){            
            ...
            $modPath = (join-path $toolsDir 'getnuget.0.0.10-beta\tools\getnuget.psm1')
            if(!(Test-Path $modPath)){
                $nugetArgs = @('install','getnuget','-prerelease','-version','0.0.10-beta','-OutputDirectory',(Resolve-Path $toolsDir).ToString())
